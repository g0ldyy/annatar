...
async def _get_stream_for_torrent(
    ...
    torrent: Optional[Torrent] = await db.get(f"torrent:{info_hash}", model=Torrent)
    if not torrent:
...
async def get_stream_for_torrent(
    ...
    cached_stream: Optional[StreamLink] = await db.get(cache_key, model=StreamLink)
    if cached_stream:
    ...
    await db.put(cache_key, sl, ttl=timedelta(weeks=1))
    return sl
